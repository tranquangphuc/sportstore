...
const products = ({ category }, { db }) => ({
  totalSize: () =>
    db
      .get('products')
      .filter(p =>
        category ? new RegExp(category, 'i').test(p.category) : true
      )
      .size.value(),
  products: ({ page, pageSize, sort }) => {
    let query = db.get('products');
    if (category)
      query = query.filter(p => new RegExp(category, 'i').test(p.category));
...
const orders = ({ onlyUnshipped = false }, { db }) => ({
  totalSize: db
    .get('orders')
    .filter(o => (onlyUnshipped ? o.shipped === false : true))
    .size()
    .value(),
